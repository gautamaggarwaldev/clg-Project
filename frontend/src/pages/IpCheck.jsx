...
import { Shield, Zap, Download, Eye, EyeOff, Loader2, AlertTriangle, CheckCircle, XCircle, Info, TrendingUp, Globe, Lock } from "lucide-react";
...
const IpCheck = () => {
  ...
  const handleAIAnalysis = async () => {
    if (!report) {
      alert("Please scan an IP address first before running AI analysis.");
      return;
    }
    ...
    try {
      ...
      const response = await fetch("https://cs-qhmx.onrender.com/v1/ai/analyze-report", {
        ...
        body: JSON.stringify({
          ...
          meta: {
            ...
            asn: report.ipInfo.asn,
            asOwner: report.ipInfo.asOwner,
      ...
      if (data.success && data.analysis) {
        ...
        setAiReport(`Based on the IP address scan results, here's my analysis:

Threat Assessment:
The IP address ${ip} shows ${report.stats.malicious > 0 ? 'moderate to high' : 'low'} risk with ${report.stats.malicious} malicious and ${report.stats.suspicious} suspicious detections.

Key Findings:
- ${report.stats.malicious} security engines detected malicious activity
- ${report.stats.suspicious} engines flagged the IP as suspicious
- ${report.stats.harmless} engines found it harmless
- Location: ${report.ipInfo.country || 'Unknown'}
- Network: ${report.ipInfo.asOwner || 'Unknown'}

Security Recommendations:
- ${report.stats.malicious > 0 ? 'Block this IP address from your network' : 'Monitor this IP address for suspicious activity'}
- Implement firewall rules to restrict access
- Consider geolocation-based blocking if applicable
- Monitor network traffic from this source

Overall Security Score: ${Math.max(0, 100 - (report.stats.malicious * 15 + report.stats.suspicious * 5))}/100

This analysis is based on real-time threat intelligence from multiple security engines and global threat databases.`);
      }
    } catch (err) {
...
The AI analysis service is currently unavailable. Here's a summary based on the IP scan results:
...
Please try the AI analysis again in a few moments, or proceed with the standard IP report above.`);
    } finally {
      setAiLoading(false);
    }
  };
    ...
    try {
      ...
      return paragraphs.map((paragraph, index) => {
        ...
        if (trimmedPara.endsWith(':') && trimmedPara.length < 100) {
          return (
            <h4 key={index} className="text-lg font-semibold text-purple-400 mb-3 mt-4 first:mt-0">
              {trimmedPara}
            </h4>
          );
        }
  ...
  const generatePDF = () => {
    ...
    // Generate HTML for PDF - Same design as ScanURL and DomainCheck
    const htmlContent = `
      <!DOCTYPE html>
      <html>
      <head>
        <title>IP Address Security Scan Report</title>
        <style>
      ...
      <body>
        <div class="header">
          <h1>⚡ IP Address Security Scan Report</h1>
        ...
        <div class="section">
          ...
          <div class="info-grid">
            <div class="info-item">
              ...
              <div class="info-value">${report.ipInfo.country || 'N/A'}</div>
            </div>
            <div class="info-item">
              <div class="info-label">ASN</div>
              <div class="info-value">${report.ipInfo.asn || 'N/A'}</div>
            </div>
            <div class="info-item">
              <div class="info-label">Network</div>
              <div class="info-value">${report.ipInfo.network || 'N/A'}</div>
            </div>
            <div class="info-item">
              <div class="info-label">Registry</div>
              <div class="info-value">${report.ipInfo.registry || 'N/A'}</div>
            </div>
            <div class="info-item">
              <div class="info-label">AS Owner</div>
              <div class="info-value">${report.ipInfo.asOwner || 'N/A'}</div>
            </div>
            <div class="info-item">
            ...
            <div class="info-item">
              ...
              <div class="info-value">${formatDate(report.last_analysis_date)}</div>
            </div>
          </div>
        </div>
        ...
        <div class="footer">
          <p><strong>IP Security Scanner</strong> - Advanced Threat Detection Report</p>
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        >
          ...
          <h1 className="text-4xl md:text-6xl font-bold mb-3">
            <span className="bg-clip-text text-transparent bg-gradient-to-r from-purple-400 via-violet-500 to-purple-400 animate-gradient">
              IP Analysis
            </span>
          </h1>
        ...
        >
          <div className="relative">
            <div className="absolute -inset-1 bg-gradient-to-r from-purple-500 to-violet-500 rounded-2xl blur opacity-25" />
              ...
              <div className="flex flex-col md:flex-row gap-4">
                <div className="flex-1 relative">
                  ...
                  <input
                    ...
                    className="w-full p-4 pl-12 pr-4 rounded-xl bg-gray-800/50 border border-gray-700 focus:border-purple-500 focus:ring-2 focus:ring-purple-500/50 outline-none transition-all text-white placeholder-gray-500"
                  />
                </div>
                <motion.button
                  onClick={handleScan}
                  className="flex items-center justify-center gap-2 bg-gradient-to-r from-purple-600 to-violet-600 hover:from-purple-500 hover:to-violet-500 text-white py-4 px-8 rounded-xl font-semibold shadow-lg transition-all relative overflow-hidden group"
                  whileHover={{ scale: 1.02 }}
                  whileTap={{ scale: 0.98 }}
                  disabled={loading}
                >
                  <div className="absolute inset-0 bg-gradient-to-r from-purple-400 to-violet-400 opacity-0 group-hover:opacity-20 transition-opacity" />
                  {loading ? (
                    <>
            ...
            >
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                >
                  <div className="flex items-center gap-2 mb-6">
                    <Globe className="w-5 h-5 text-purple-400" />
                    <h3 className="text-xl font-semibold text-gray-200">IP Information</h3>
                  </div>
                  <div className="space-y-4">
                    <div>
                      <p className="text-gray-500 text-sm mb-2">Target IP</p>
                      <p className="text-purple-400 font-mono text-sm break-all bg-gray-800/50 p-3 rounded-lg border border-gray-700">
                        {ip}
                      </p>
                    </div>
                    <div>
                    ...
                    <div>
                      <p className="text-gray-500 text-sm mb-2">Country</p>
                      <p className="text-gray-300 font-medium">{report.ipInfo.country || 'N/A'}</p>
                    </div>
                    <div>
                      <p className="text-gray-500 text-sm mb-2">AS Owner</p>
                      <p className="text-gray-300 font-medium">{report.ipInfo.asOwner || 'N/A'}</p>
                    </div>
                  </div>
                </motion.div>
              </div>
